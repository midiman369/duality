...
# Setup burst at t=0 (ONESTOP, bank 0/0). PCs are 1-based in the table.
setup = {1: 89, 2: 51, 3: 100, 4: 101, 5: 76, 6: 61, 7: 49, 8: 1, 9: 37, 10: 1,
         11: 48, 12: 123, 13: 63, 14: 88, 15: 74, 16: 30}
for i, (ch, pc) in enumerate(setup.items()):
    t = 0.001 * i
    ev(t, mido.Message("control_change", channel=ch - 1, control=0, value=0))
    ev(t, mido.Message("control_change", channel=ch - 1, control=32, value=0))
    ev(t, mido.Message("program_change", channel=ch - 1, program=pc - 1))

rng = random.Random(7)
# Pad ch1: overlapping sustained chords from 3 s on (always sounding).
t = 0.05
while t < 60:
    for n in (48, 55, 60, 64):
        note(t, 0, n, 4.3, 70)
    t += 4.0
# Strings ch2/7/11 from 3 s.
for ch in (1, 6, 10):
    t = 3.0 + ch * 0.01
    while t < 60:
        note(t, ch, 60 + rng.randrange(12), 1.9, 75); t += 2.0
# Wind ch5 phrases 5–30 s with gaps.
t = 5.0
while t < 30:
    note(t, 4, 72 + rng.randrange(7), 0.4); t += 0.5 + (2.0 if rng.random() < 0.2 else 0)
# FX ch3 at 9 s, PC → 61 (orch brass) at 25 s, then brass.
for k in range(6): note(9 + k, 2, 70, 0.8)
ev(25.0, mido.Message("program_change", channel=2, program=60))
for k in range(10): note(26 + k * 1.5, 2, 67, 1.0)
# Lead ch14 and guitar ch16 from 15 s (the 21:17:40 moment).
for k in range(60): note(15 + k * 0.5, 13, 76 + (k % 5), 0.35)
for k in range(80): note(15.2 + k * 0.4, 15, 52 + (k % 3), 0.3)
# ch8 piano, then PC 31 dist guitar at 20 s (the 21:17:46 second dirt box).
for k in range(8): note(3.5 + k * 2, 7, 60, 1.0)
ev(20.0, mido.Message("program_change", channel=7, program=30))
for k in range(60): note(21 + k * 0.5, 7, 45, 0.3)
# ch12 gunshots (no family) and ch10 drums.
for k in range(20): note(4 + k * 2.7, 11, 60, 0.2)
t = 3.0
while t < 60:
    note(t, 9, 36 if int(t * 2) % 2 == 0 else 38, 0.1); t += 0.25
# Synth brass ch13 late.
for k in range(10): note(40 + k * 1.2, 12, 60, 0.9)
...
for t, p, typ in type_writes:
    ...
    last_w[p] = t
fails += hitches[:10]
for c in (15, 13):   # guitar ch16, lead ch14 must get a box soon after they play
