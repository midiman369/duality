...
IN = common.midi("bass-in-000745.mid")
...
innotes=set((round(t,2),m.note) for t,m in events if m.type=="note_on" and m.velocity and m.channel==0)
pcs=[None]*6; bad=0; hero=0; sub=0
for t,p,m in REC:
    if getattr(m,"channel",None)!=0: continue
    if m.type=="program_change": pcs[p]=m.program
    if m.type=="note_on" and m.velocity:
        if (round(t-1000,2),m.note) in innotes or any(abs(round(t-1000,2)-a)<=0.02 and n==m.note for a,n in innotes):
            hero+=1
            if pcs[p]!=32: bad+=1; print("  FILE NOTE WITH WRONG TONE", round(t-1000,2), "P%d"%(p+1), "n",m.note, "pc",pcs[p])
        else:
            sub+=1
print("file bass notes",hero,"wrong tone",bad,"| sub notes",sub)
